...
- **전투 배속 버튼**
  - 전투 화면 하단 컨트롤 바에 `x0.5 / x1 / x2 / x4` 배속 버튼 추가.
  - 버튼 클릭으로 즉시 게임 진행 속도 전환 가능(기본값 x1).
  - UI 가독성을 위해 배속 버튼과 일시정지 버튼 위치를 소폭 상향 조정.
  - 고배속에서 투사체가 적을 관통해 히트 판정이 지연/누락되던 문제를 보정(이동 거리 기반 충돌 처리).
  - 후반 웨이브(61+)에서는 처치 기반 궁극기 충전 효율이 단계적으로 감소해, 궁극기 연속 난사가 과도하게 발생하지 않도록 밸런스 조정.
